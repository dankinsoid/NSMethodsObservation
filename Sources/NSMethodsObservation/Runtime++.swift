...
#if !os(Linux)
import Foundation
#if canImport(NSMethodsObservationObjc) && SWIFT_PACKAGE && !DISABLE_SWIZZLING && !os(Linux)
...
extension NSObjectProtocol {
	...
	@discardableResult
	public func onSentMessage(_ selector: Selector, action: @escaping ([Any]) -> Void) throws -> Cancellation {
		try synchronized(on: self) {
			...
			return Cancellation { proxy.messageSent[id] = nil }
		}
	}
	
	@discardableResult
	public func onMethodInvoked(_ selector: Selector, action: @escaping ([Any]) -> Void) throws -> Cancellation {
		try synchronized(on: self) {
			// in case of dealloc selector replay subject behavior needs to be used
			if selector == deallocSelector {
				return onDeallocated { action([]) }
			}
			
			let proxy: MessageSentProxy = try self.registerMessageInterceptor(selector)
			let id = UUID()
			proxy.methodInvoked[id] = action
			return Cancellation { proxy.methodInvoked[id] = nil }
		}
	}
	
	@discardableResult
	public func onDeallocating(_ action: @escaping () -> Void) throws -> Cancellation {
		try synchronized(on: self) {
